...
import { getYourGuideSearchUrl } from "@/lib/getYourGuideLinks";
import { CityItinerary } from "@/types/itinerary";
...
/**
 * Extracts the most interesting, bookable-sounding activities from the
 * itinerary and presents them as a calm editorial list.
 */
function deriveCuratedTours(
  itinerary: CityItinerary,
  cityName: string,
  country: string,
  userInterests: string[]
  ...
  const candidates: CuratedTour[] = [];

  // Collect must-do and interest-aligned activities
      ...
      for (const activity of slot.activities) {
        ...
        const isRelevant =
          ...
          userInterests.some((i) =>
            activity.category?.toLowerCase().includes(i.toLowerCase())
          );

        if (isRelevant) {
          candidates.push({
            title: activity.title,
            rationale: activity.description.length > 100
              ? activity.description.slice(0, 97) + "…"
              : activity.description,
            url: getYourGuideSearchUrl(activity.title, cityName, country),
          });
        }
      }
    }
...
export const CuratedToursSection = ({
  itinerary,
  cityName,
  country,
  userInterests,
}: CuratedToursSectionProps) => {
  const tours = deriveCuratedTours(itinerary, cityName, country, userInterests);
