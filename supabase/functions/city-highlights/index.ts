...
Respond with ONLY valid JSON in this exact format:
{
  ...
  "perfectDayTimeline": [
    ...
    {"time": "Evening", "activity": "As golden light hits Humayun's Tomb, join locals on the lawns..."},
    {"time": "Night", "activity": "End at Hauz Khas Village for rooftop drinks overlooking the reservoir..."}
  ],
  "perfectDayNarrative": "A single paragraph fallback version of the timeline above...",
...
2. Exactly 3 personalMatchReasons as objects with "placeName" (short 2-5 word place/experience name) and "reason" (one sentence connecting it to the traveler's interest). Each must reference a specific place in ${requestData.city} that connects to one of the traveler's interests: ${humanInterests}. Write each reason as plain prose with the interest name naturally embedded — no bold, no brackets, no placeholders.
...
8. A hero image search query for the city`;
    ...
    const content = await callClaude(systemPrompt, userPrompt, { model: SONNET });
